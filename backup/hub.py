...
class Hub:
    def __init__(self, delay_ms, fps, detector=None, detect_fps=10):
...
if __name__ == '__main__':
    ...
    a=p.parse_args(); logging.basicConfig(level=logging.DEBUG if a.verbose else logging.INFO, format='%(asctime)s %(levelname)s %(message)s')
    ...
    web.run_app(Hub(a.delay_ms,a.fps,detector,a.detect_fps).app(),host=a.host,port=a.port,ssl_context=tls_context(cert,key),access_log=None)
